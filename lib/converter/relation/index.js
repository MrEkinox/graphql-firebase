...
const relationToFirestore = async (input, targetName, batch, currentData = []) => {
    ...
    return [...currentData, ...addRef, ...created]
        .filter((newData) => { var _a; return !((_a = input.remove) === null || _a === void 0 ? void 0 : _a.includes(newData.id)); })
        .reduce((acc, cur) => {
...
const relationFromFirestore = async (refs, targetName, whereInput) => {
    ...
    const documents = await (0, firebase_admin_1.firestore)().getAll(...refs);
    return async_1.default.map(documents, async (doc) => (0, collection_1.collectionTargetFromFirestore)(doc, target));
};
