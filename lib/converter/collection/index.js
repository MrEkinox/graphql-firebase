...
const collectionFromFirestore = async (targetName, input, whereInput, orderByInput, root, info) => {
    ...
    if (whereInput) {
        const test = (0, firebase_admin_1.firestore)()
            .collectionGroup("documents")
            .where("id", "==", whereInput === null || whereInput === void 0 ? void 0 : whereInput.documents.id.equalTo);
        const test2 = await test.get();
        console.log("test", test2.docs.map((doc) => doc.data()));
    }
    if (orderByInput) {
