...
class Converter {
    constructor(schema, batch) {
        this.fileListToFirestore = async (field, input, ref) => {
            ...
            if (addedFiles.length)
                this.batch.set(ref, { [field.name]: firebase_admin_1.firestore.FieldValue.arrayUnion(...addedFiles) }, { merge: true });
            if (link.length)
                this.batch.set(ref, { [field.name]: firebase_admin_1.firestore.FieldValue.arrayUnion(...link) }, { merge: true });
            if (remove.length)
                this.batch.update(ref, {
                    [field.name]: firebase_admin_1.firestore.FieldValue.arrayRemove(...remove),
                });
        };
    ...
    async convertReferenceList(field, input, ref) {
        ...
        if (created.length) {
            this.batch.set(ref, { [field.name]: firebase_admin_1.firestore.FieldValue.arrayUnion(...created) }, { merge: true });
        }
        const addRef = (input.add || []).map((id) => collection.doc(id));
        if (addRef.length)
            this.batch.set(ref, { [field.name]: firebase_admin_1.firestore.FieldValue.arrayUnion(...addRef) }, { merge: true });
        const removeRef = (input.remove || []).map((id) => collection.doc(id));
        if (removeRef.length) {
            this.batch.update(ref, {
                [field.name]: firebase_admin_1.firestore.FieldValue.arrayRemove(...removeRef),
            });
        }
    }
}
