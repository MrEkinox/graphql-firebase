...
class Converter {
    constructor(schema, batch) {
        this.fileListToFirestore = async (field, input, ref) => {
            ...
            if (addedFiles.length)
                this.batch.set(ref, { [field.name]: admin.firestore.FieldValue.arrayUnion(...addedFiles) }, { merge: true });
            if (link.length)
                this.batch.set(ref, { [field.name]: admin.firestore.FieldValue.arrayUnion(...link) }, { merge: true });
            if (remove.length)
    ...
    async convertReferenceList(field, input, ref) {
        ...
        if (created.length) {
            this.batch.set(ref, { [field.name]: admin.firestore.FieldValue.arrayUnion(...created) }, { merge: true });
        }
        const addRef = (input.add || []).map((id) => collection.doc(id));
        if (addRef.length)
            this.batch.set(ref, { [field.name]: admin.firestore.FieldValue.arrayUnion(...addRef) }, { merge: true });
