...
class Converter {
    ...
    async toFirebase(name, newData, parentRef, snapshot) {
        ...
        const resultArray = await Promise.all(fields.map(async (field) => {
            ...
            if (field.type === "Reference") {
                ...
                fieldData = await this.convertReferenceList(field, fieldData, lastFieldData);
            }
    ...
    async convertCollection(field, input, parentRef) {
        ...
        await async_1.default.map((input === null || input === void 0 ? void 0 : input.update) || [], async (data2) => {
            ...
            const data = await this.toFirebase(field.target, data2.fields, docRef, snapshot);
    ...
    async convertReferenceList({ target }, input, lastData = []) {
        if (!target)
            return [];
        const parents = (0, mutations_1.getParents)(target, [], this.schema);
        const collection = (0, mutations_1.getCollection)(parents);
