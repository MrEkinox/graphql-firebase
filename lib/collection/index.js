...
const collectionResolver = async (type, fieldName, parents, src, _a, info) => {
    var _b;
    var { where, limit, offset, orderBy } = _a, input = __rest(_a, ["where", "limit", "offset", "orderBy"]);
    ...
    if (where) {
        const whereCollection = new where_1.WhereCollection(info.schema, parentsIds, orderBy);
    ...
    const count = (0, utils_1.hasCountField)(info)
        ? (await collection.count().get()).data().count
        : 0;
    const documents = (0, utils_1.hasEdgeField)(info) ? (await collection.get()).docs : [];
    const edges = (0, where_1.sortOrderBy)(documents.map((doc) => ({
