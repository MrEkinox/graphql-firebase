...
exports.WhereCollection = void 0;
...
class WhereCollection {
    constructor(schema) {
        this.getWhereInput = (name, input, parent) => {
            ...
            return fields.reduce((acc, field) => {
                ...
                if (field.type === "Collection" && field.target && fieldInput) {
                    const inField = this.getWhereInput(field.target, fieldInput, newParent);
                    return [...acc, ...inField];
                }
                return acc;
            }, newInput ? [newInput] : []);
        ...
        this.whereReferenceId = (field, input, collection) => {
            ...
            if (!whereField || !whereOperator || !field.target || !whereID)
                return collection;
            const targetCollection = (0, mutations_1.getCollection)(field.target, []);
    ...
    async getData(collection) {
        ...
        const edges = data.docs.map((doc) => ({
            node: Object.assign({ id: doc.id }, doc.data()),
        }));
        return { count, edges };
    }
    ...
    removeCollectionFields(name, input, parent) {
        const fields = (0, utils_1.getSchemaFields)(name, this.schema);
    ...
    async getCollectionWhere(whereInput, ids = []) {
        ...
        if (!where)
            return ids;
        const collectionGroup = (0, firebase_admin_1.firestore)().collectionGroup((0, utils_1.plural)(where.name));
        ...
        const snapIds = snapshot.docs.map((doc) => {
            ...
            const index = where.parent
                ? path.indexOf((0, utils_1.plural)(where.parent))
                : path.indexOf((0, utils_1.plural)(where.name));
